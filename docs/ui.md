...
5. **玻璃只给 Input。** 主表面与栏都不用玻璃；chip 是描边胶囊。
...
| `outline` | 0.20 | chip 描边、键帽描边 |
| `hairline` | 0.12 | 保留的档位，当前无使用者（R11 去掉了最后一条分割线） |
| `selection` | 0.10 | Row Focus 选中块；chip 选中填充 |
...
| `row` | 10 | 选中块、组头 hover |
| `keyCap` | 6 | 底栏按键提示的键帽 |
| chip | 高度 / 2 | 胶囊（不是令牌，由高度决定） |

窗口本身的圆角由系统决定，不设令牌。

### 2.3 尺寸与间距 `Theme.Size` / `Theme.Spacing`

间距阶梯：2 / 4 / 6 / 8 / 10 / 12 / 16 / 20。尺寸：窗口默认 720×480、最小 480×320；Input 高 48；Scope Bar 高 36；底栏高 36；chip 高 26、内边距 22、间距 8；内容缩进 16；文字轨 = 16 + 11 − 2 = 25；行竖向内边距 10；行间距 6；组头高 28。R10 起全部在 `Theme.Size` / `Theme.Spacing`（字号与行高仍在 `Preferences`，因为逻辑测试 target 用到）。
...
- **选中块不改变文字颜色，且与 Input 同宽。** 自绘选中（10% 墨色、圆角 10）的左右边缘就是 Input 胶囊的边缘（窗口坐标里各 `contentInset`），按窗口而不是按表格宽度算，滚动条出不出现都不动；`interiorBackgroundStyle` 保持 `.normal`，不出现白字。
- **滚动条永远是 overlay。** `OverlayScrollView` 无视系统首选样式（连鼠标会切成 legacy）：传统滚动条会在透明表面上画不透明轨道，还会让表格变窄。
- **右侧光学轨：** 时间列右缘、组头 chevron 中心、Scope Bar 的 "+" 中心对齐——chevron 的 nudge 由 "+" chip 的真实宽度（symbol 槽 16 + 内边距 22）算出。
- **chip 选中不改宽度**（字重不变），描边 `outline`，选中填 `selection`。
...
- **左：功能组**（从左到右）Resolved 切换 · Sort（Newest ⇄ Priority 两态切换，无菜单） · Conflicts（有冲突时出现） · Trash。全部是 `ScopeChipButton`（保留自绘，R7 结论）。
- **右：按键提示组**，按当前焦点状态切换，每条 = 键帽 + 动作名（`Theme.Typography.hint`，`Ink.tertiary`；键帽：`Ink.selection` 填充、`Ink.outline` 描边、圆角 6、`.caption2` medium）：
  - Input Focus：`↵ Create` · `↓ List`
  - Row Focus：`↵ Edit` · `␣ Resolve` · `⌫ Trash`（`←→ Priority` 在宽度允许时追加；Move 没有快捷键，右键或拖拽）
  - Trash 表面：`↵ Restore` · `⌫ Delete` · `esc Back`
- 提示组的压缩优先级最低：窗口窄到放不下时先隐藏提示，功能组永远可见。离线标签放在提示组左侧，同样可压缩。
- 提示是给用户看的，不是可点击控件（不拦截鼠标，`refusesFirstResponder`）。

## 6. 列表行

- 选中块：`Ink.selection`，圆角 `Radius.row`，左右边缘 = Input 胶囊边缘（窗口坐标 `contentInset`），纵向占整行（含行间距的一半）；只在 Row Focus 存在——Input 取得焦点时清空选区（R9）。

...
2. **自绘是最后手段，但要量**：只有平台控件无法表达 PRD 语义（例如 chip 选中不改宽度、`refusesFirstResponder` 保住 Input 焦点）或实测明显更贵时才自绘，并在 ARCHITECTURE §8 登记"为什么"。替换自绘前先做控件微基准（HISTORY 决策 14）——实测系统 bezel 的首绘/重绘可能比轻量自绘贵一个量级。
